...
class Select1(object):
    ...
    def select_by_visible_partial_text(self, text):
        """Select all options that display text matching the argument. That is, when given "Bar" this
           would select an option like:
           ...
           :Args:
            - text - The partial text to match against

            throws NoSuchElementException If there is no option with specisied text in SELECT
           """
        xpath = "//option[contains(normalize-space(),%s)]" % self._escapeString(text)
        opts = self._el.find_elements(By.XPATH, xpath)
        matched = False
        for opt in opts:
            self._setSelected(opt)
        ...
        if not matched:
            raise NoSuchElementException("Could not locate element with partial text: %s" % text)
